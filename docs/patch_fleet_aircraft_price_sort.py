...
targets = [Path("src/js/fleet-market-table-dialog.js"), Path("src/js/fleet.js")]
helper = r"""
function aircraftPurchasePriceValue(row) {
  const candidates = [row.new_purchase_price, row.base_purchase_price, row.purchase_price, row.estimated_new_price, row.catalog_price, row.price_amount, row.new_cost_amount, row.price];
...
function sortAircraftByPurchasePrice(rows) {
  return [...rows].sort((a, b) => {
    ...
    return String(a.icao_type_code || a.model_code || a.model_name || "").localeCompare(String(b.icao_type_code || b.model_code || b.model_name || ""));
  });
}
"""
for path in targets:
    ...
    if 'function sortAircraftByPurchasePrice(' not in text:
        ...
        text = text.replace(marker, helper+marker, 1) if marker in text else text+'
'+helper
    for old,new in [
    ...
    if text != original:
        path.write_text(text,encoding='utf-8'); print(f"OK: patched {path}")
    else: print(f"OK: no changes needed for {path}")
