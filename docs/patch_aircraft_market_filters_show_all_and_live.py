...
DOC = ROOT / 'docs/AIRCRAFT_MARKET_FILTERS_SHOW_ALL_AND_LIVE.md'

for path in (CATALOG, DIALOG, FILTERS):
    if not path.exists():
        raise SystemExit(f'Missing file: {path}')

# Ensure catalog exposes the real DB fields used by the client-side filters.
...
missing_fields = [field for field in ('aircraft_family', 'engine_type', 'aircraft_category') if field not in select_prefix]
if missing_fields:
    anchors = [
        '      operation_role,\n',
        '    operation_role,\n',
    ...
    for anchor in anchors:
        if anchor in catalog:
            catalog = catalog.replace(anchor, anchor + ''.join(f'      {field},\n' for field in missing_fields), 1)
            inserted = True
            break
    if not inserted:
        raise SystemExit('Could not safely add aircraft_family/engine_type/aircraft_category to catalog SELECT')
CATALOG.write_text(catalog)

# Ensure market rows carry filter data attributes. If they already exist, keep them.
js = DIALOG.read_text()
if 'data-aircraft-engine=' not in js or 'data-aircraft-family=' not in js:
    # Patch the first plain aircraft row template. This matches the current market table renderer style.
    pattern = '            <tr>\n'
    replacement = (
        '            <tr\n'
...
if 'detailRow("Engine"' not in js and 'detailRow("Role", model.operation_role)' in js:
    js = js.replace(
...
FILTERS.write_text(r'''(() => {
  ...
  function refreshFamilyOptions() {
    ...
    const nextHtml = '<option value="">All families</option>' + families
      .map(family => `<option value="${escapeHtml(family)}">${escapeHtml(familyLabel(family))}</option>`)
      .join("");

    if (familySelect.innerHTML !== nextHtml) {
      familySelect.innerHTML = nextHtml;
    }
...
DOC.write_text('''# Aircraft market filters show all and live filtering
...
print('Patched Buy new aircraft filters to show all rows by default and filter live.')
