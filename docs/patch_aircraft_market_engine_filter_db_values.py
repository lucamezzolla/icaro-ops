...
DOC = ROOT / 'docs/AIRCRAFT_MARKET_ENGINE_FILTER_DB_VALUES.md'

for path in (CATALOG, DIALOG, FILTERS):
    if not path.exists():
        raise SystemExit(f'Missing file: {path}')

# 1) Ensure catalog.php returns engine_type from aircraft_models.
...
# Remove references to dropped fields if still present in SELECT.
catalog = re.sub(r'^[ \t]*aircraft_family,\s*\n', '', catalog, flags=re.MULTILINE)
catalog = re.sub(r'^[ \t]*aircraft_category,\s*\n', '', catalog, flags=re.MULTILINE)
CATALOG.write_text(catalog)

# 2) Make the market row store the real DB engine_type in data attributes.
js = DIALOG.read_text()

# Remove obsolete family/category data attributes and detail rows from older patches.
js = re.sub(r'\n\s*data-aircraft-family="[^\n]*"', '', js)
js = re.sub(r'\n\s*data-aircraft-category="[^\n]*"', '', js)
js = re.sub(r'\n\s*\$\{detailRow\("Family",\s*model\.aircraft_family\)\}', '', js)
js = re.sub(r'\n\s*\$\{detailRow\("Category",\s*model\.aircraft_category\)\}', '', js)

engine_attr = '              data-aircraft-engine="${escapeHtml(row.engine_type || row.engineType || row.engine || "")}"\n              data-engine-type="${escapeHtml(row.engine_type || row.engineType || row.engine || "")}"'

if 'data-aircraft-engine=' in js:
    js = re.sub(
        r'\n\s*data-aircraft-engine="[^\n]*"(?:\n\s*data-engine-type="[^\n]*")?',
        '\n' + engine_attr,
        js,
        count=1,
    )
    ...
    raise SystemExit('Could not safely find aircraft market <tr> to add engine_type data attributes')

# Ensure search text includes engine_type but not removed fields.
js = re.sub(
...
# Ensure details show Engine once.
...
# 3) Rewrite filters as a DB engine_type based live filter.
FILTERS.write_text(r'''(() => {
  ...
  // UI values map directly to aircraft_models.engine_type.
  // The legacy value JET is kept only as a group/fallback.
  const ENGINE_GROUPS = {
  ...
  function ensureFiltersOnBuyDialog() {
    ...
    if (filters) {
      ensureEngineOptions(filters);
      removeObsoleteFamilyFilter(filters);
      return;
    }
    ...
    filters.innerHTML = `
      <div class="aircraft-market-filter-grid">
        ...
        <label>
          <span>Engine</span>
          <select id="aircraftFilterEngine"></select>
        </label>
  ...
  function ensureEngineOptions(filters) {
    const select = filters.querySelector("#aircraftFilterEngine");

    if (!select) {
      return;
    }

    const currentValue = normalizeCode(select.value);
    const html = `
    ...
    if (["", "JET", "TURBOFAN", "TURBOPROP", "PISTON", "TURBOSHAFT", "SUPERSONIC"].includes(currentValue)) {
      select.value = currentValue;
    }
  }

  function removeObsoleteFamilyFilter(filters) {
    const family = filters.querySelector("#aircraftFilterFamily");
    const label = family?.closest?.("label");
  ...
  function extractAircraftRowData(row) {
    ...
    const engine = normalizeCode(
      row.dataset.engineType
      || row.dataset.aircraftEngine
      || row.getAttribute("data-engine-type")
      || row.getAttribute("data-aircraft-engine")
      || ""
    );
  ...
  function engineMatches(actualEngine, selectedEngine) {
    const actual = normalizeCode(actualEngine);
    const selected = normalizeCode(selectedEngine);

    if (!selected) {
      return true;
    }

    const allowed = ENGINE_GROUPS[selected] || [selected];
...
DOC.write_text('''# Aircraft market engine filter DB values
...
print('Patched Buy new aircraft Engine filter to use aircraft_models.engine_type values.')
